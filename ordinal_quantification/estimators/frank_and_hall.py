...
from ..metrics.binary import binary_kld
from .utils import FHLabelBinarizer
from .utils import check_and_correct_probabilities_asc
...
class FrankAndHallClassifier(BaseEstimator, ClassifierMixin):
    ...
    def __fit_binary(self, X, y_bin, pos_class):
    ...
    def fit(self, X, y):
        ...
        self.estimators_ = Parallel(n_jobs=self.n_jobs)(delayed(self.__fit_binary)(X, y_bin_fh[:, pos_class], pos_class)
...
class FrankAndHallMonotoneClassifier(FrankAndHallClassifier):
    ...
    def predict_proba(self, X):
        ...
        predictions_left = check_and_correct_probabilities_asc(predictions_left)
